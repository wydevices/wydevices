                              ...
                              const char *method)
{
    ...
    uint8_t hash[16];
    char *authstr;
    AVRandomState ramdomstate;

    digest->nc++;
    snprintf(nc, sizeof(nc), "%08x", digest->nc);

    /* Generate a client nonce. */
    av_init_random(0xdeadbeef, &ramdomstate);
    for (i = 0; i < 2; i++)
        cnonce_buf[i] = av_random(&ramdomstate);
    ff_data_to_hex_lower(cnonce, (const uint8_t*) cnonce_buf, sizeof(cnonce_buf));
    ...
    ff_data_to_hex_lower(A1hash, hash, 16);
    A1hash[32] = 0;
    ...
    av_md5_final(md5ctx, hash);
    ff_data_to_hex_lower(A2hash, hash, 16);
    A2hash[32] = 0;
    ...
    av_md5_final(md5ctx, hash);
    ff_data_to_hex_lower(response, hash, 16);
    response[32] = 0;
