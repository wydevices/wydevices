...
class RecordingList(object):
	...
	def GetConflict(self, oRecording, ignore_not_scheduled=False):
		if ignore_not_scheduled:
			schedule_or_running = filter((lambda rec : rec.type != RECORDING_TYPE_NOT_SCHEDULED and (rec.status == RECORDING_STATUS_RUNNING or rec.status == RECORDING_STATUS_SCHEDULED)), self.recordings.values())
		else:
			schedule_or_running = filter((lambda rec : rec.status == RECORDING_STATUS_SCHEDULED or rec.status == RECORDING_STATUS_RUNNING), self.recordings.values())
	...
	def GetConflictForNotScheduledRecording(self, oRecording, include_instant_record=False):
		if include_instant_record:
			schedule_or_running = filter((lambda rec: rec.status == RECORDING_STATUS_RUNNING or rec.status == RECORDING_STATUS_RUNNING), self.recordings.values())
		else:
			schedule_or_running = filter((lambda rec: rec.type != RECORDING_TYPE_NOT_SCHEDULED and (rec.status == RECORDING_STATUS_RUNNING or rec.status == RECORDING_STATUS_SCHEDULED)), self.recordings.values())
		self.chrono.orderRecording(schedule_or_running)
