...
function build_subpackages() {
	...
	for PKG in $SUBPACKAGES; do
		...
		cat $PKGDIR/usr/etc/versions \
			$SUBPKGDIR/usr/etc/versions 2>/dev/null \
			| grep -v "#" > $PKGDIR/usr/etc/versions.aux

		cp -r $SUBPKGDIR/usr $PKGDIR
		mv $PKGDIR/usr/etc/versions.aux $PKGDIR/usr/etc/versions
...
#	generate_tar	
	...
	# version file
	mkdir -p $PKGDIR/usr/etc/
	if [ -f $PKGDIR/usr/etc/versions ]; then
		mv $PKGDIR/usr/etc/versions $PKGDIR/usr/etc/versions.subpkgs
		echo "# Built on `date -R`" > $PKGDIR/usr/etc/versions
		echo "#" >> $PKGDIR/usr/etc/versions
		echo $NAME-$VERSION >> $PKGDIR/usr/etc/versions
		cat $PKGDIR/usr/etc/versions.subpkgs | awk '{ print "|-- "$0 }' >> $PKGDIR/usr/etc/versions
		rm -f $PKGDIR/usr/etc/versions.subpkgs
	else
		echo "# Built on `date -R`" > $PKGDIR/usr/etc/versions
		echo "#" >> $PKGDIR/usr/etc/versions
		echo $NAME-$VERSION >> $PKGDIR/usr/etc/versions
