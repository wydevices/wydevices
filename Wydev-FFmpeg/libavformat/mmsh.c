...
static int read_data_packet(MMSHContext *mmsh, const int len)
{
    ...
    av_log(NULL, AV_LOG_ERROR, "Data packet len = %d\n", len);
    if (res != len) {
        ...
        if (res < 0) {
            ...
            if (!mms->header_parsed) {
                if (mms->asf_header) {
                    if (len != mms->asf_header_size) {
                        ...
                        av_log(NULL, AV_LOG_ERROR, "Header len changed from %d to %d\n",
                                mms->asf_header_size, len);
            ...
            if (len) {
                ...
                if (res != len) {
                    ...
                    av_log(NULL, AV_LOG_ERROR, "Skip chunk type %d \n", chunk_type);
                    continue;
                }
            }
        }
...
static int mmsh_open(URLContext *h, const char *uri, int flags)
{
    ...
    url_join(httpname, sizeof(httpname), "http", NULL, host, port, path);
    ...
    av_log(NULL, AV_LOG_ERROR, "out_buffer is %s", headers);
    ff_http_set_headers(mms->mms_hd, headers);
    ...
    av_log(NULL, AV_LOG_DEBUG, "Connection successfully open\n");
    return 0;
fail:
    av_freep(&stream_selection);
    mmsh_close(h);
    av_log(NULL, AV_LOG_ERROR, "Connection failed with error %d\n", err);
    return err;
}
