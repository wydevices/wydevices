...
static void print_guid(const ff_asf_guid *g)
{
    ...
    PRINT_IF_GUID(g, ff_asf_header);
        ...
        av_log(NULL, AV_LOG_DEBUG, "(GUID: unknown) ");
    for(i=0;i<16;i++)
...
static void get_tag(AVFormatContext *s, const char *key, int type, int len)
{
    ...
    if (type == 0) {         // UTF16-LE
        get_str16le(s->pb, len, value, 2*len + 1);
...
static int asf_read_ext_content_desc(AVFormatContext *s, int64_t size)
{
    ...
    for(i=0;i<desc_count;i++) {
        ...
        if (name_len%2)     // must be even, broken lavf versions wrote len-1
            name_len += 1;
        if ((ret = get_str16le(pb, name_len, name, sizeof(name))) < name_len)
...
static int asf_read_language_list(AVFormatContext *s, int64_t size)
{
    ...
    for(j = 0; j < stream_count; j++) {
        ...
        if ((ret = get_str16le(pb, lang_len, lang, sizeof(lang))) < lang_len)
...
static int asf_read_marker(AVFormatContext *s, int64_t size)
{
    ...
    for(i=0;i<count;i++){
        ...
        if ((ret = get_str16le(pb, name_len * 2, name, sizeof(name))) < name_len)
            url_fskip(pb, name_len - ret);
...
static int asf_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    for(;;) {
        ...
        ff_get_guid(pb, &g);
        gsize = get_le64(pb);
        av_log(s, AV_LOG_DEBUG, "%08"PRIx64": ", gpos);
        print_guid(&g);
        av_log(s, AV_LOG_DEBUG, "  size=0x%"PRIx64"\n", gsize);
...
static int ff_asf_get_packet(AVFormatContext *s, ByteIOContext *pb)
{
    ...
    if (rsize > packet_length - padsize) {
        ...
        av_log(s, AV_LOG_ERROR,
               ...
               rsize, packet_length, padsize, url_ftell(pb));
        return -1;
    }
    ...
    av_log(s, AV_LOG_DEBUG, "packet: size=%d padsize=%d  left=%d\n", s->packet_size, asf->packet_padsize, asf->packet_size_left);
    return 0;
}
