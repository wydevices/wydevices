...
{
    MMSContext *mms = &mmst->mms;
    av_log(NULL, AV_LOG_ERROR, "Stream changing!\n");

    // 40 is the packet header size, 7 is the prefix size.
    mmst->header_packet_id= AV_RL32(mms->in_buffer + 40 + 7);
    av_log(NULL, AV_LOG_ERROR, "Changed header prefix to 0x%x", mmst->header_packet_id);
}
...
static MMSSCPacketType get_tcp_server_response(MMSTContext *mmst)
{
    ...
    for(;;) {
        ...
        if(AV_RL32(mms->in_buffer + 4)==0xb00bface) {
            ...
            av_log(NULL, AV_LOG_ERROR, "Length remaining is %d\n", length_remaining);
            ...
            if(packet_id_type == mmst->header_packet_id) {
                ...
                av_log(NULL, AV_LOG_ERROR, "packet id type %d is old.", packet_id_type);
                continue;
            }
        }
                              ...
                              const MMSSCPacketType expect_type)
{
    ...
    if(send_fun) {
        ...
        if (ret < 0) {
            av_log(NULL, AV_LOG_ERROR, "Send Packet error before expecting recv packet %d\n", expect_type);
            return ret;
        }
    }
...
static int mms_open(URLContext *h, const char *uri, int flags)
{
    ...
    url_join(tcpname, sizeof(tcpname), "tcp", NULL, mmst->host, port, NULL);
    ...
    if (err) {
        av_log(NULL, AV_LOG_ERROR, "asf header parsed failed!\n");
        goto fail;
    }
    mms->header_parsed = 1;
    ...
    av_log(NULL, AV_LOG_ERROR, "Leaving open (success)\n");
    return 0;
fail:
    mms_close(h);
    av_log(NULL, AV_LOG_ERROR, "Leaving open (failure: %d)\n", err);
    return err;
}
        ...
        if(mms->asf_header_read_size < mms->asf_header_size) {
            ...
            if (err == 0) {
                if(mms->remaining_in_len>mms->asf_packet_len) {
                    ...
                    result = ff_mms_read_data(mms, buf, size);
                    if (result == 0) {
                        av_log(NULL, AV_LOG_ERROR, "read asf media paket size is zero!\n");
                        break;
                    }
                }
            } else {
                av_log(NULL, AV_LOG_ERROR, "read packet error!\n");
                break;
            }
        }
